...
def get_email_thread_from_instantly(lead_email, email_account):
    """
    Fetch all emails for a lead from Instantly (v2), sort ascending,
    and return them concatenated by thread_id and timestamp.
    """
    url = "https://api.instantly.ai/api/v2/emails"
    params = {"lead": lead_email, "limit": 100, "sort_order": "asc"}
    headers = {
    ...
    for tid, msgs in threads.items():
        ...
        for m in msgs:
            ...
            sub  = m.get("subject", "")
            body = (m.get("body", {}).get("text") or m.get("plain_body", "")).strip()
...
def instantly_webhook():
    ...
    if payload.get("event_type") == "campaign_completed":
        ...
        return jsonify(status="created-campaign", item=data["data"]["create_item"]["id"]), 201

    # --- Only handle email_sent events below ---
    ...
    # --- 1) Find existing item by lead_email via items_page_by_column_values ---
    find_query = """
    ...
    app.logger.debug("Monday find response [%s]: %s", find_resp.status_code, find_resp.text)
    find_resp.raise_for_status()

    items = (find_resp.json()
                  .get("data", {})
                  .get("items_page_by_column_values", {})
                  .get("items", []))

    # Column IDs for updates
    ...
    if items:
        ...
        for cv in item["column_values"]:
            if cv["id"] == LONG_TEXT_COL:
                try:
                    ...
                    current = val.get("text", "")
                except:
                    current = cv["value"] or ""

        new_thread = (current + "\n---\n" if current else "") + email_thread
        ...
        update_resp = requests.post(
            "https://api.monday.com/v2",
            headers=HEADERS,
            json={"query": update_mutation, "variables": {"itemId": item_id, "columnVals": json.dumps(update_vals)}}
        )
        ...
        new_id = create_resp.json()["data"]["create_item"]["id"]
        return jsonify(status="created", item=new_id), 201
